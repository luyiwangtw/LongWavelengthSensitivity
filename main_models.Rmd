...
```{r, fig.cap="Figure caption: Relative irradiance of the standard daylight used for daylight models"}
...
##for plotting two relative irradiance together.
# irradiance.twilight.plot <- read.csv("data/civil twilight.csv",header=TRUE) %>% 
#   as.rspec(lim=c(300,800)) %>% 
#   procspec(opt = c("min", "max")) %>% 
#   rename( twilight = Irradiance) #absolute irradiance; this is just for plotting purpose
# 
# illum <- merge(irradiance.d65, irradiance.twilight.plot) %>% gather(light, illuminace, -wl) 
# 
...
#Plot D65 irradiance together with twilight irradiant
#Problem: when convert Watt to photon, the shape changes over wavlengths because photon=watt*lambda*5.05*10^15
#Not comparable to plot the two together
# mag.twilight <- 1/0.04340328
# irra.twilight.plot <- irradiance.twilight["wl"] %>% cbind(irradiance.twilight["twilight"]*mag.twilight) %>% as.rspec()
# illum <- merge(irradiance.d65, irra.twilight.plot) %>% gather(light, illuminace, -wl)
# 
# ggplot(illum, aes(x=wl, y=illuminace, color=light))+
#   geom_line()+
#   xlab("Wavelength (nm)")+
...
#     sec.axis = sec_axis( trans=~.*(1/mag.twilight), name="Photon flux of twilight")) +
#   theme_classic()
```

```{r, fig.cap="Figure caption: Photon flux of the twilight used for twilight models"}

ggplot(irradiance.twilight, aes(x=wl, y=twilight))+
  geom_line()+
  xlab("Wavelength (nm)")+
  scale_y_continuous(name = "Photon flux (umol/s/m^2)")+
  theme_classic()
```
</br>
</br>
