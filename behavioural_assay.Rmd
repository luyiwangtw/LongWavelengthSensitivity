...
#re-scale the average leaf reflectacne
aveleaf <- read.csv("data/aveleaf.csv")
aveleaf$attracted <- (aveleaf$aveleaf)/60
aveleaf <- aveleaf %>% 
  rename(
    wavelength = wl
    )
aveleaf <- aveleaf[59:93,]
datacombined<- ymaze.Cdimidiata %>% rbind(ymaze.Cflavopicta)
ggplot(datacombined, aes(x=wavelength, y=attracted))+
  geom_jitter(data=ymaze.Cflavopicta,height=0.01,width=5, alpha=0.5, size= 3, color="#FC6A03")+
  geom_jitter(data=ymaze.Cdimidiata,height=0.01,width=5, alpha=0.5, size= 3, color="#153E7E")+
  geom_ribbon(data=newdat1.Cf, alpha=0.3,fill="#FC6A03", aes(ymax=phi.Cf, ymin=plo.Cf))+
  geom_ribbon(data=newdat1.Cd, alpha=0.3, fill="#153E7E", aes(ymax=phi.Cd, ymin=plo.Cd))+ 
  geom_line(data=newdat1.Cf, aes(x=wavelength, y=attracted), color="#FC6A03")+
  geom_line(data=newdat1.Cd, aes(x=wavelength, y=attracted), color="#153E7E")+ 
  geom_hline(yintercept=0.50, color="grey60")+
  geom_line(data=aveleaf, color="#008000", linetype = "dashed")+
  scale_x_continuous(breaks = seq(580, 780, by = 20))+
  scale_y_continuous(sec.axis = sec_axis(~.*(6/10), name = "Reflectance"))+
  theme_classic()
# ggplot(datacombined, aes(x=wavelength, y=attracted))+
